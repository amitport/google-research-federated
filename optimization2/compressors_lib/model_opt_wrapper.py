...
def kashin_quantization(bits):
  return te.core.EncoderComposer(
  #   BitpackingEncodingStage(bits)
  # ).add_parent(
  #   te.stages.UniformQuantizationEncodingStage(bits),
...
def _create_kashin_fn(value_type, bits, p):
  # breakpoint()
  # concat_type = tff.tf_computation(compression_utils.flatten_concat, value_type).type_signature.result
  # as_simple_encoder(kashin_quantization(bits), tff.types.type_to_tf_tensor_specs(concat_type))
  enc = kashin_quantization(bits)#Enc_Wrapper(kashin_quantization(bits))
