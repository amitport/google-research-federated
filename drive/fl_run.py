...
from distributed_dp.fl_utils import get_total_dim
from drive.drive_utils import build_simdrive_factory
...
def create_1m_cnn_model(only_digits: bool = False, seed: Optional[int] = 0):
  ...
  gradients / deltas concatenated as a single vector to be padded to the
...
def main(argv):
  ...
  hadamard_repeats = 1
  aggregation_factory = build_simdrive_factory(bits=bits, hadamard_repeats=hadamard_repeats)
  write_hparams({'bits': bits,
                 'hadamard_repeats': hadamard_repeats,
                 'dim': get_total_dim(model_trainable_variables)})
