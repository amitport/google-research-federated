...
def no_replace_random_choice(n, k):
  '''numpy.random.choice(n, size=k, replace=False)'''
...
def drive_roundtrip(input_record,
  ...
  dequantized_record = inverse_drive_quantization(quantized_record, scale,
                                                  bits=bits)
...
def drive_roundtrip_builder(bits=4, hadamard_repeats=1):
  # @tf.function
  def drive_roundtrip2(input_record):
    """Applies compression to the record as a single concatenated vector."""
    input_vec = compression_utils.flatten_concat(input_record)
    ...
    timestamp_microseconds = tf.cast(tf.timestamp() * microseconds_per_second,
                                     tf.int32)
    sample_hadamard_seed = tf.convert_to_tensor([timestamp_microseconds, 0])
...
def drive_sub_roundtrip(input_record,
  ...
  rotated_record = compression_utils.randomized_hadamard_transform(
    sparse_record, seed_pair=hadamard_seed, repeat=hadamard_repeats)
...
def drive_sub_roundtrip2(input_record,
                        hadamard_seed: tf.Tensor,
                        rand_p_seed: tf.Tensor,
                        p=0.1,
                        bits=1,
  ...
  rotated_record = compression_utils.randomized_hadamard_transform(
    casted_record, seed_pair=hadamard_seed, repeat=hadamard_repeats)
  ...
  quantized_record, scale = drive_quantization(sparse_record, bits=bits)
  dequantized_record = inverse_drive_quantization(quantized_record, scale,
                                                  bits=bits)
...
def _create_drive_fn(value_type, bits, hadamard_repeats):
  ...
  def drive_fn(record):
    ...
    timestamp_microseconds = tf.cast(tf.timestamp() * microseconds_per_second,
                                     tf.int32)
    sample_hadamard_seed = tf.convert_to_tensor([timestamp_microseconds, 0])
    return drive_roundtrip(record, sample_hadamard_seed=sample_hadamard_seed,
                           bits=bits,
...
def _create_drive_sub_fn(value_type, bits, p, hadamard_repeats):
  @tff.tf_computation(value_type)
  def drive_fn(record):
    microseconds_per_second = 10 ** 6  # Timestamp returns fractional seconds.
    timestamp_microseconds = tf.cast(tf.timestamp() * microseconds_per_second,
    ...
    return drive_sub_roundtrip2(record,
...
def _next_fn_impl(state,
                  ...
                  inner_agg_process,
                  weight=None):
  reconstructed_value = tff.federated_map(drive_fn, value)
  if weight is None:
    inner_agg_output = inner_agg_process.next(state, reconstructed_value)
  else:
    inner_agg_output = inner_agg_process.next(state, reconstructed_value,
                                              weight)
...
def _weighted_simdrive_factory(inner_agg_factory, bits, hadamard_repeats):
  class WeightedSimdriveFactory(tff.aggregators.WeightedAggregationFactory):
    def create(self, value_type,
               weight_type) -> tff.templates.AggregationProcess:
      drive_fn = _create_drive_fn(value_type, bits=bits,
                                  hadamard_repeats=hadamard_repeats)
      inner_agg_process = inner_agg_factory.create(
        drive_fn.type_signature.result, weight_type)
      init_fn = inner_agg_process.initialize

      @tff.federated_computation(
        init_fn.type_signature.result,
        tff.types.at_clients(value_type),
        tff.types.at_clients(weight_type))
      def next_fn(state, value, weight):
        return _next_fn_impl(state, value, drive_fn, inner_agg_process, weight)

      return tff.templates.AggregationProcess(init_fn, next_fn)

  return WeightedSimdriveFactory()


def _unweighted_simdrive_factory(inner_agg_factory, bits, hadamard_repeats):
  """Creates a unweighted factory for flattening and concatenation."""

  class UnweightedSimdriveFactory(tff.aggregators.UnweightedAggregationFactory):
    ...
    def create(self, value_type) -> tff.templates.AggregationProcess:
      drive_fn = _create_drive_fn(value_type, bits=bits,
                                  hadamard_repeats=hadamard_repeats)
      inner_agg_process = inner_agg_factory.create(
        drive_fn.type_signature.result)
      init_fn = inner_agg_process.initialize
      state_type = init_fn.type_signature.result

      @tff.federated_computation(
        state_type, tff.types.at_clients(value_type))
      def next_fn(state, value):
        return _next_fn_impl(state, value, drive_fn, inner_agg_process)

      return tff.templates.AggregationProcess(init_fn, next_fn)

  return UnweightedSimdriveFactory()


def build_simdrive_factory(
  bits, hadamard_repeats,
  inner_agg_factory: Optional[tff.aggregators.AggregationFactory] = None) -> \
  tff.aggregators.AggregationFactory:
  if inner_agg_factory is None:
    inner_agg_factory = tff.aggregators.MeanFactory()
  if isinstance(inner_agg_factory, tff.aggregators.WeightedAggregationFactory):
    return _weighted_simdrive_factory(inner_agg_factory, bits=bits,
                                      hadamard_repeats=hadamard_repeats)
  else:
    return _unweighted_simdrive_factory(inner_agg_factory, bits=bits,
                                        hadamard_repeats=hadamard_repeats)
