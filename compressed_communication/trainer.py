...
with utils_impl.record_hparam_flags() as shared_flags:
    ...
    flags.DEFINE_integer('bits', 1, 'Quantization bits.')
...
def main(argv: Sequence[str]) -> None:
    ...
    if FLAGS.aggregator == 'no_compression':
        ...
        aggregation_factory = builder.build_rotation_ablation_aggregator(
            ...
            clipping=FLAGS.clipping,
            weighted=FLAGS.weighted_averaging)
    elif FLAGS.aggregator == 'hadamard_sq':
        aggregation_factory = builder.build_rotation_aggregator(
            bits=FLAGS.bits,
            rotation='hadamard',
            concatenate=FLAGS.concatenate,
            zeroing=FLAGS.zeroing,
        ...
        aggregation_factory = builder.build_eden_aggregator(bits=FLAGS.bits)
    elif FLAGS.aggregator == 'quic_fl':
        aggregation_factory = builder.build_quic_fl_aggregator(bits=FLAGS.bits)
    ...
    asyncio.get_event_loop().run_until_complete(
        asyncio.gather(
            *[m.release(test_metrics, FLAGS.total_rounds + 1) for m in metrics_managers]))
