...
from compressed_communication.aggregators.comparison_methods.eden import EdenFactory
from compressed_communication.aggregators.comparison_methods.minmax_sq import MinMaxSQFactory
from compressed_communication.aggregators.comparison_methods.quic_fl.quic_fl import QuicFLFactory
from compressed_communication.aggregators.min_max_normalization import MinMaxNormalizationFactory
...
def build_rotation_aggregator(
    ...
    # factory = quantize.QuantizeFactory(2**-bits,
    #                                    entropy.EntropyFactory(include_zeros=True),
    #                                    rounding_type)
    # factory = MinMaxNormalizationFactory(factory)
    factory = MinMaxSQFactory(bits)
    return configure_aggregator(factory, rotation, concatenate, zeroing, clipping,
                                weighted)


def build_eden_aggregator(bits: int = 1) -> tff.aggregators.AggregationFactory:
    factory = EdenFactory(bits)
    return configure_aggregator(factory, "identity", True, False, False, True)


def build_quic_fl_aggregator(bits: int = 1) -> tff.aggregators.AggregationFactory:
    factory = QuicFLFactory(bits)
    return configure_aggregator(factory, "identity", True, False, False, True)
